locals {
  ...
  pge_artifactory_dev_url     = "https://cae-artifactory.jpl.nasa.gov/artifactory/${var.artifactory_repo}/gov/nasa/jpl/nisar/sds/pcm/pge_snapshots/${var.pge_snapshots_date}"
  pge_artifactory_release_url = "https://cae-artifactory.jpl.nasa.gov/artifactory/${var.artifactory_repo}/gov/nasa/jpl/nisar/sds/pge/"
  ...
  cnm_response_queue_name = {
    "dev"  = "nisar-dev-daac-cnm-response"
    "int"  = "nisar-int-daac-cnm-response"
    "test" = "nisar-test-daac-cnm-response"
  }
  cnm_response_dl_queue_name = {
    "dev"  = "nisar-dev-daac-cnm-response-dead-letter-queue"
    "int"  = "nisar-int-daac-cnm-response-dead-letter-queue"
    "test" = "nisar-test-daac-cnm-response-dead-letter-queue"
  }
...
resource "aws_cloudwatch_metric_alarm" "sqs_cnm_r_dead_letter_alarm" {
  ...
  insufficient_data_actions = []
  alarm_actions             = [aws_sns_topic.cnm_response_dead_letter_queue[count.index].arn]
  dimensions = {
...
resource "aws_cloudwatch_metric_alarm" "sqs_dead_letter_alarm" {
  ...
  insufficient_data_actions = []
  alarm_actions             = [aws_sns_topic.isl_dead_letter_sns.arn]
  dimensions = {
...
resource "aws_cloudwatch_metric_alarm" "sqs_event_misfire_alarm" {
  ...
  insufficient_data_actions = []
  alarm_actions             = [aws_sns_topic.event_misfire_sns.arn]
  dimensions = {
...
# sns
######################
resource "aws_sns_topic" "isl_dead_letter_sns" {
  name = "${var.project}-${var.venue}-${local.counter}-isl-sns"
}

resource "aws_sns_topic_policy" "isl_dead_letter_sns" {
  depends_on = [aws_sns_topic.cnm_response, data.aws_iam_policy_document.sns_topic_policy]
  arn        = aws_sns_topic.isl_dead_letter_sns.arn
  policy     = data.aws_iam_policy_document.isl_dead_letter_sns_topic_policy.json
}

resource "aws_sns_topic" "event_misfire_sns" {
  name = "${var.project}-${var.venue}-${local.counter}-event-misfire-sns"
}

resource "aws_sns_topic_policy" "event_misfire_sns" {
  depends_on = [aws_sns_topic.event_misfire_sns, data.aws_iam_policy_document.event_misfire_sns_topic_policy]
  arn        = aws_sns_topic.event_misfire_sns.arn
  policy     = data.aws_iam_policy_document.event_misfire_sns_topic_policy.json
}

data "aws_iam_policy_document" "event_misfire_sns_topic_policy" {
  depends_on = [aws_sns_topic.event_misfire_sns]
  ...
  statement {
    ...
    resources = [
      aws_sns_topic.event_misfire_sns.arn
    ]
    sid = "__default_statement_ID"
  }
}

data "aws_iam_policy_document" "isl_dead_letter_sns_topic_policy" {
  depends_on = [aws_sns_topic.isl_dead_letter_sns]
  policy_id  = "__default_policy_ID"
  statement {
    actions = [
      "SNS:Publish",
    ...
    resources = [
      aws_sns_topic.isl_dead_letter_sns.arn
    ]
    sid = "__default_statement_ID"
  }
}
...
# SNS Topic for CNM-R Dead Letter Queue
resource "aws_sns_topic" "cnm_response_dead_letter_queue" {
  count = local.sqs_count
  name  = "${var.project}-${var.venue}-${local.counter}-cnm-response-dead-letter-queue"
}

resource "aws_sns_topic_policy" "cnm_response_dead_letter_sns" {
  count      = local.sqs_count
  depends_on = [aws_sns_topic.cnm_response_dead_letter_queue, data.aws_iam_policy_document.cnm_response_dead_letter_sns_topic_policy]
  arn        = aws_sns_topic.cnm_response_dead_letter_queue[count.index].arn
  policy     = data.aws_iam_policy_document.cnm_response_dead_letter_sns_topic_policy[count.index].json
}

data "aws_iam_policy_document" "cnm_response_dead_letter_sns_topic_policy" {
  count      = local.sqs_count
  depends_on = [aws_sns_topic.cnm_response_dead_letter_queue]
  policy_id  = "__default_policy_ID"
  statement {
    actions = [
      "SNS:Publish",
    ...
    resources = [
      aws_sns_topic.cnm_response_dead_letter_queue[count.index].arn
    ]
    sid = "__default_statement_ID"
  }
}
...
resource "aws_lambda_function" "isl_lambda" {
  depends_on    = [null_resource.download_lambdas, aws_sns_topic.isl_dead_letter_sns]
  ...
  environment {
    variables = {
      ...
      "JOB_RELEASE"         = var.nisar_pcm_branch
      ...
      "SIGNAL_FILE_SUFFIX"  = "{\"tlm\": {\"ext\": \".qac\", \"checksum_type\": \"md5\"},\"met_required\":{\"ext\":\".signal\"}}"
      "ISL_SNS_TOPIC"       = aws_sns_topic.isl_dead_letter_sns.arn
  ...
  */
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo LDAP_GROUPS: nisar-pcm-dev >> ~/.sds/config",
      ...
      "echo '  JOB_RELEASE: ${var.nisar_pcm_branch}' >> ~/.sds/config",
      ...
      "echo EXTRACTOR_HOME: /home/ops/verdi/ops/nisar-pcm/extractor >> ~/.sds/config",
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "if [ \"${var.use_artifactory}\" = true ]; then",
      "  fab -f ~/.sds/cluster.py -R mozart,grq,metrics,factotum update_nisar_packages",
      "else",
      "  fab -f ~/.sds/cluster.py -R mozart,grq,metrics,factotum,verdi update_nisar_packages",
      "fi",
      "if [ \"${var.grq_aws_es}\" = true ] && [ \"${var.use_grq_aws_es_private_verdi}\" = true ]; then",
      ...
      "aws s3 cp logstash-7.1.1.tar.gz s3://${local.code_bucket}/",
      "sds -d reset all -f",
      ...
      "pip install -e .",
      /*
      "if [[ \"${var.nisar_pge_release}\" == \"develop\"* ]]; then",
      "    python ~/mozart/ops/nisar-pcm/tools/deploy_pges.py --pge_release \"${var.nisar_pge_release}\" --image_names ${var.pge_names} --sds_config ~/.sds/config --processes 4 --force --artifactory_url ${local.pge_artifactory_dev_url}",
      "else",
      "    python ~/mozart/ops/nisar-pcm/tools/deploy_pges.py --pge_release \"${var.nisar_pge_release}\" --image_names ${var.pge_names} --sds_config ~/.sds/config --processes 4 --force --artifactory_url ${local.pge_artifactory_release_url}",
      "fi",
      */
  ...
  provisioner "remote-exec" {
    inline = [
      "set -ex",
      "source ~/.bash_profile",
      "wget https://cae-artifactory.jpl.nasa.gov/artifactory/${var.artifactory_repo}/gov/nasa/jpl/nisar/sds/pge/testdata_R1.0.0/l0b_small_001.tgz!/input/id_06-00-0101_chirp-parameter_v44.12.xml -O /export/home/hysdsops/mozart/ops/nisar-pcm/tests/pge/l0b/id_06-00-0101_chirp-parameter_v44.12.xml",
      "wget https://cae-artifactory.jpl.nasa.gov/artifactory/${var.artifactory_repo}/gov/nasa/jpl/nisar/sds/pge/testdata_R1.0.0/l0b_small_001.tgz!/input/id_01-00-0101_radar-configuration_v44.12.xml -O /export/home/hysdsops/mozart/ops/nisar-pcm/tests/pge/l0b/id_01-00-0101_radar-configuration_v44.12.xml",
      "wget https://cae-artifactory.jpl.nasa.gov/artifactory/${var.artifactory_repo}/gov/nasa/jpl/nisar/sds/pge/testdata_R1.0.0/l0b_small_001.tgz!/input/id_ff-00-ff01_waveform.xml -O /export/home/hysdsops/mozart/ops/nisar-pcm/tests/pge/l0b/id_ff-00-ff01_waveform.xml",
    ]
  }
